...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dynamically load shared header/footer components using ES6 modules
    const loadWrappers = () => {
        return import('./components/common/header.js')
            .then(headerModule => import('./components/common/footer.js')
                .then(footerModule => ({ headerModule, footerModule }))
            )
            .then(({ headerModule, footerModule }) => {
                // Initialize header and footer
                const headerContainer = document.getElementById('header');
                const footerContainer = document.getElementById('footer');
                
                if (headerContainer && headerModule.createHeader) {
                    headerModule.createHeader(headerContainer, {
                        title: 'GovHealthcare',
                        logo: 'hospital',
                        fixed: true
                    });
                ...
                if (footerContainer && footerModule.createFooter) {
                    footerModule.createFooter(footerContainer);
                }
            })
            .catch(error => {
                console.error('Error loading header/footer modules:', error);
            });
    };
